...
class TestRun:
    ...
    def test_run_no_submit(
        self, m_lrd, m_btrd, m_bds, m_sj, m_ssdj, sep_xios_server, xios_servers, tmpdir
    ):
        p_run_dir = tmpdir.ensure_dir("run_dir")
        p_results_dir = tmpdir.ensure_dir("results_dir")
        m_lrd.return_value = {
...
class TestSubmitJob:
    ...
    def test_submit_job(
        ...
        m_run.assert_called_once_with(
            [
                queue_job_cmd,
                "{run_dir}/SalishSeaNEMO.sh".format(run_dir=Path("run_dir")),
            ],
            check=True,
    ...
    def test_submit_job_w_waitjob(
        ...
        m_run.assert_called_once_with(
            [
                queue_job_cmd,
                depend_flag,
                "{depend_option}:42".format(depend_option=depend_option),
                "{run_dir}/SalishSeaNEMO.sh".format(run_dir=Path("run_dir")),
            ],
            check=True,
            universal_newlines=True,
        ...
        assert m_run.call_args_list == [
            call(
                [
                    queue_job_cmd,
                    depend_flag,
                    "{depend_option}:43".format(depend_option=depend_option),
